...
## Generates a review for the most recently active customer using the assembled ingredient,
## appends it to the NPC's notepad as an auto_review entry, and instantiates a Review UI node.
func _generate_customer_review() -> void:
	...
	customer.add_auto_review(customer.given_review)
	var new_review: Node = REVIEW.instantiate()
